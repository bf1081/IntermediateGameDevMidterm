...
public class progress_bar_Cyris_BIIIITHC : MonoBehaviour {
	...
	void FixedUpdate () {
		if (Shop.isIn == true && BasicLootCrates.isIn == false) {
			time_To_Load = 8;
		}
		if (Shop.isIn == false && BasicLootCrates.isIn == true) {
			time_To_Load = 3;
		}
		...
		if (Input.GetKey (KeyCode.E) && Fill_Amount_Cyrus < 0.99 && BasicLootCrates.isIn == true || Input.GetKey (KeyCode.E) && Fill_Amount_Cyrus < 0.99 && Shop.isIn == true) {
